...
import { X, Crown, UserPlus, LogOut, Users, Search, Check, ChevronLeft, Trash2 } from 'lucide-react';
...
const GroupInfo = ({ group, onClose, onMembersUpdated }) => {
  ...
  const handleAddMember = async (targetUser) => {
    ...
    try {
      ...
      if (onMembersUpdated) {
        onMembersUpdated([
          ...group.members,
          { id: targetUser.id, name: targetUser.username || targetUser.email, role: 'member' },
        ]);
      }
    } catch (err) {
  ...
  const handleLeaveClick = () => setConfirm({ type: 'leave', title: 'Keluar Grup', message: 'Yakin ingin keluar dari grup ini?' });
  const handleDeleteClick = () => setConfirm({ type: 'delete', title: 'Hapus Grup', message: 'Hapus grup ini? Semua pesan akan dihapus permanen.' });
  ...
  const handleRoleToggle = async (member) => {
    ...
    try {
      ...
      toast.success(newRole === 'admin' ? `${member.name} dijadikan admin` : `${member.name} diturunkan ke member`);
    } catch (err) {
      const msg = err?.response?.data?.error?.message;
      ...
      {showAddPanel ? (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-1">
            ...
            {searchStatus !== 'loading' && searchResults.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-6">Tidak ada pengguna</p>
            )}
            {searchResults.map((u) => {
              ...
              return (
                ...
                >
                  <Avatar name={u.username || u.email} size="sm" online={u.is_online} />
        ...
        <div className="flex-1 overflow-y-auto scrollbar-light">
          ...
          <div className="flex flex-col items-center py-8 px-4">
            ...
            <h2 className="text-lg font-bold text-slate-900 text-center">{group.name}</h2>
            <p className="text-sm text-slate-500 mt-1">{group.members?.length} anggota</p>
            {group.description && (
