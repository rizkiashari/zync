...
import { Send, Smile, Paperclip, X, FileText, CornerUpLeft, Image } from 'lucide-react';
...
const FilePreview = ({ file, onRemove }) => {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border-b border-slate-100">
      {isImage ? (
        <img src={url} alt={file.name} className="w-12 h-12 object-cover rounded-lg border border-slate-200" />
      ) : (
...
const MessageInput = ({ onSend, onTyping, replyTo, onCancelReply, roomId }) => {
  ...
  const handleSend = async () => {
    ...
    if (attachedFile && roomId) {
      ...
      try {
        ...
        const fileMeta = JSON.stringify({ _type: 'file', url, name, mime, size });
  ...
  const canSend = (text.trim().length > 0 || attachedFile !== null) && !uploading;

  return (
    <div className="border-t border-slate-100 bg-white flex-shrink-0">
      {replyTo && <ReplyBar replyTo={replyTo} onCancel={onCancelReply} />}
      {attachedFile && <FilePreview file={attachedFile} onRemove={() => !uploading && setAttachedFile(null)} />}
