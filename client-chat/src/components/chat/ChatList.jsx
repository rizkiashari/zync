...
const formatTime = (dateStr) => {
  ...
  if (hr < 24) return d.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
    ...
    >
      ...
      <div className="flex-1 min-w-0 text-left">
        <div className="flex items-center justify-between mb-0.5">
          <span className={`text-sm font-medium truncate ${isActive ? 'text-white' : 'text-slate-100'}`}>
            {displayName}
          </span>
          <span className={`text-xs flex-shrink-0 ml-2 ${unread > 0 ? 'text-indigo-300' : 'text-slate-500'}`}>
        ...
        <div className="flex items-center justify-between">
          <p className={`text-xs truncate flex-1 ${unread > 0 ? 'text-slate-300 font-medium' : 'text-slate-500'}`}>
            {isGroup && room.member_count > 0 && (
              <span className="text-slate-400">{room.member_count} anggota · </span>
            )}
...
const ChatList = ({ rooms, activeTab, searchQuery }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-thin px-2 py-1 space-y-0.5">
      {(activeTab === 'all' || activeTab === 'messages') && directRooms.length > 0 && (
        <>
          {activeTab === 'all' && (
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3 py-2">Pesan</p>
          )}
          {directRooms.map(room => (
            <ChatListItem key={room.id} room={room} isActive={isActive(room)} onClick={() => handleClick(room)} />
          ))}
        </>
      )}
      ...
      {(activeTab === 'all' || activeTab === 'groups') && groupRooms.length > 0 && (
        <>
          {activeTab === 'all' && directRooms.length > 0 && (
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3 py-2 mt-2">Grup</p>
          )}
          {activeTab === 'all' && directRooms.length === 0 && (
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3 py-2">Grup</p>
          )}
          {groupRooms.map(room => (
            <ChatListItem key={room.id} room={room} isActive={isActive(room)} onClick={() => handleClick(room)} />
          ))}
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-slate-500">
          <p className="text-sm">{searchQuery ? 'Tidak ada hasil' : 'Belum ada percakapan'}</p>
